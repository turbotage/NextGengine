...
void ngv::VulkanSwapChain::setup(const vk::PhysicalDevice& newPhysicalDevice, const vk::Device& newDevice, const vk::Queue& newQueue, uint32 newGraphicsQueueIndex)
{
...
void ngv::VulkanSwapChain::setSurface(const vk::SurfaceKHR& newSurface)
{
    surface = newSurface;
...
void ngv::VulkanSwapChain::create(vk::Extent2D& size, bool vsync)
{
...
std::vector<vk::Framebuffer> ngv::VulkanSwapChain::createFramebuffers(vk::FramebufferCreateInfo framebufferCreateInfo)
{
...
vk::ResultValue<uint32_t> ngv::VulkanSwapChain::acquireNextImage(const vk::Semaphore& presentCompleteSemaphore, const vk::Fence& fence)
{
...
void ngv::VulkanSwapChain::clearSubmitFence(uint32_t index)
{
    images[index].fence = vk::Fence();
}

vk::Fence ngv::VulkanSwapChain::getSubmitFence(bool destroy)
{
...
vk::Result ngv::VulkanSwapChain::queuePresent(vk::Semaphore waitSemaphore)
{
...
void ngv::VulkanSwapChain::destroy()
{
    for (uint32_t i = 0; i < imageCount; i++) {
        device.destroyImageView(images[i].view);
    }
...
uint32_t ngv::VulkanSwapChain::findQueue(const vk::QueueFlags& flags) const
{
